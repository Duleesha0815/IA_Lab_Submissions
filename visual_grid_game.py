...
class VisualGridHuntGame:
    ...
    def __init__(self, width=10, height=10, num_food=10, num_opponents=2, custom_walls=None):
        ...
        # ========== PRACTICAL 01 ADDITION: Toxic Traps ==========
        self.toxic_traps = set()
        num_traps = 3  # feel free to change this number
        while len(self.toxic_traps) < num_traps:
            tx = random.randint(0, self.width - 1)
            ty = random.randint(0, self.height - 1)
            pos_tuple = (tx, ty)
            # Avoid (0,0), walls, food, and opponent starting positions
            opponent_tuples = [tuple(op) for op in self.opponents]
            if (pos_tuple != (0, 0) and
                pos_tuple not in self.walls and
                pos_tuple not in self.food_positions and
                pos_tuple not in opponent_tuples):
    ...
    def get_percept(self) -> dict:
        return {
            ...
            # ========== PRACTICAL 01 ADDITION: Toxin sensor ==========
    ...
    def execute_action(self, action: str):
        ...
        # ========== PRACTICAL 01 ADDITION: Toxic trap penalty ==========
...
class GridGameGUI:
    ...
    def draw_grid(self):
        ...
        # ========== PRACTICAL 01 ADDITION: Draw toxic traps (purple) ==========
        for tx, ty in self.env.toxic_traps:
            offset = self.cell_size * 0.3
            x1 = tx * self.cell_size + offset
            y1 = (self.env.height - 1 - ty) * self.cell_size + offset
            # Draw a diamond / polygon shape in purple
            self.canvas.create_polygon(
                x1, y1 + self.cell_size * 0.5,
                x1 + self.cell_size * 0.5, y1,
                x1 + self.cell_size, y1 + self.cell_size * 0.5,
